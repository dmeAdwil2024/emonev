...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    ...
    <body>
									...
									<div class="col-md-12">
										<div class="form-group row">
											<div class="col-12 p-2">
												<label for="tgl_mulai" class="control-label normal">File {{$mdl_nm}}</label>
                                				<input type=file name="fileimport" id="fileimport" class="form-control normal"/>
				                            </div>
										</div>
										<div>
											<br>
												<a href="javascript:f_import()" class="btn btn-success btn-sm p-2" style="border-radius:50px;"><font style="font-size:12pt;font-weight:550;color:#fff">&nbsp;&nbsp;Import&nbsp;&nbsp;</font></a>&nbsp;
												...
												<a  href="{{env('APP_URL')}}/sakti/{{$mdl_file}}" class="btn btn-info btn-sm p-2" style="border-radius:50px;"><font style="font-size:12pt;font-weight:550;color:#fff">&nbsp;&nbsp;Download Template&nbsp;&nbsp;</font></a>
											<br><br>
										</div>
									</div>
								</div>
		...
		<script>
			...
			function f_importx(){
				var file=$('#fileimport').prop('files')[0];
				
				var form_data = new FormData();
				form_data.append('fileimport', file);
				form_data.append('mdl', '{{$mdl}}');
				form_data.append('mdl_nm', '{{$mdl_nm}}');
				
        		form_data.append('_token', '{{csrf_token()}}')
				$.ajax({
		        url: "{{route('capaian.submit-import-sakti')}}",
		        type: "POST",
		        data: form_data,
		        cache: false,
		        processData: false,
		        ...
		        success: function (e)
		        {
					if(e.status == "success")
		            {
						...
						document.location.href="{{route('capaian.import-sakti')}}/{{$mdl}}";
		            }
		            else
		            {	
						alert(e)
