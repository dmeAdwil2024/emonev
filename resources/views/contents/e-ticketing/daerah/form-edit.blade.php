      ...
      <div class="container-fluid">
        <form>
          ...
          <div class="mb-3 row">
            ...
            <div class="form-check-inline col-1 pt-1">
              <input
                ...
                id="jenisRevisiDipa"
                value="dipa"
                {{ $data->jenis_revisi == 'dipa' ? 'checked' : '' }}
              />
          ...
          <div class="mb-3 row">
            ...
            <div class="col-10">
              <input type="text" class="form-control" name="judul_kak" id="judul_kak" value="{{ $data->judul_kak }}" />
            </div>
          </div>
          <div class="row">
            <div class="mb-3 col-6">
              <label for="nota_dinas_ppk" class="form-label">Nota Dinas PPK Sudah Sign <em>(Maksimal 10MB)</em></label>
              @if (! empty($data->nota_dinas_ppk))
              <br><a class="mb-3 d-inline-block" href="{{ route('download.dokumen', ['jenis_file' => 'nota_dinas_ppk', 'nama_file' => $data->nota_dinas_ppk]) }}">{{ $data->nota_dinas_ppk }}</a>
              @endif
              <input
                type="file"
                class="form-control"
            ...
            <div class="mb-3 col-6">
              <label for="matrik_rab" class="form-label">Matrix RAB Semula Menjadi <em>(Maksimal 10MB)</em></label>
              @if (! empty($data->matrik_rab))
              <br><a class="mb-3 d-inline-block" href="{{ route('download.dokumen', ['jenis_file' => 'matrik_rab', 'nama_file' => $data->matrik_rab]) }}">{{ $data->matrik_rab }}</a>
              @endif
              <input
                type="file"
                class="form-control"
            ...
            <div class="mb-3 col-6">
              <label for="dokumen_pendukung" class="form-label">Dokumen Pendukung Lainnya <em>(Maksimal 10MB)</em></label>
              @if (! empty($data->dokumen_pendukung))
              <br><a class="mb-3 d-inline-block" href="{{ route('download.dokumen', ['jenis_file' => 'dokumen_pendukung', 'nama_file' => $data->dokumen_pendukung]) }}">{{ $data->dokumen_pendukung }}</a>
              @endif
              <input
                type="file"
                class="form-control"
