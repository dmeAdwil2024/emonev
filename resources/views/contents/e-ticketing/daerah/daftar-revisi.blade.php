                ...
                <table class="table table-light">
                    ...
                    <tbody>
                        @foreach ($list as $i => $revisi)
                            <tr class="">
                                <td scope="row">{{ $i + 1 }}</td>
                                <td><a href="{{ $revisi->status == 'SUDAH PERBAIKAN' && Auth::user()->level == \App\UserAccess::LEVEL_KPA ? route('ticketing.view-revisi-kpa') : getLinkByStatus($revisi->status, $level) }}/{{ $revisi->id }}"
                                        class="link-primary link-underline link-underline-opacity-0 fw-semibold">{{ $revisi->nomor_surat }}</a>
                                </td>
                                ...
                                @if ($current == \App\TicketStatus::PERBAIKAN)
                                    <th scope="col"
                                        @if ($revisi->status == 'BUTUH PERBAIKAN') class="bg-danger text-white" @endif>
                                        {{ $revisi->status }}</th>
                                @endif
                            </tr>
                        @endforeach
