          ...
          <div class="mb-3 row">
            <div class="col-md-6">
              ...
              <div class="mb-3 row">
                ...
                <div class="col-8">
                  <a class="btn btn-success d-flex align-items-center" href="{{ route('download.dokumen', ['jenis_file' => 'nota_dinas_ppk', 'nama_file' => $data->nota_dinas_ppk]) }}" title="Klik untuk unduh berkas">
              ...
              <div class="mb-3 row">
                ...
                <div class="col-8">
                  <a class="btn btn-success d-flex align-items-center" href="{{ route('download.dokumen', ['jenis_file' => 'matrik_rab', 'nama_file' => $data->matrik_rab]) }}" title="Klik untuk unduh berkas">
                    <i class="fas fa-file-arrow-down h2"></i>
              ...
              <div class="mb-3 row">
                ...
                <div class="col-8">
                  <a class="btn btn-success d-flex align-items-center" href="{{ route('download.dokumen', ['jenis_file' => 'dokumen_pendukung', 'nama_file' => $data->dokumen_pendukung]) }}" title="Klik untuk unduh berkas">
                    <i class="fas fa-file-arrow-down h2"></i>
            ...
            <div class="col-md-6">
              @if (empty($data->status_fasgub) || $data->status_fasgub != \App\TicketStatus::DISETUJUI_FASGUB)
              <div class="mb-3">
                <label for="status_fasgub" class="form-label">Pilih status revisi</label>
                <select
                  class="form-select form-select-sm"
                  name="status_fasgub"
                  id="status_fasgub"
                >
                  <option value="{{ \App\TicketStatus::PERBAIKAN }}" {{ $data->status_fasgub == \App\TicketStatus::PERBAIKAN ? 'selected' : '' }}>Perbaikan</option>
                  <option value="{{ \App\TicketStatus::DISETUJUI_FASGUB }}" {{ $data->status_fasgub == \App\TicketStatus::DISETUJUI_FASGUB ? 'selected' : '' }}>Disetujui</option>
                </select>
              </div>
              <div class="mb-3 setuju">
                <label for="no_nota_fasgub" class="form-label">Nomer Nota Dinas</label>
                <input
                  type="text"
                  class="form-control"
                  name="no_nota_fasgub"
                  ...
                  aria-describedby=""
                  placeholder=""
                  value="{{ $data->no_nota_fasgub }}"
                />
              </div>
              <div class="mb-3 setuju">
                <label for="lampiran_fasgub" class="form-label">Nota Dinas fasgub</label>
                @if (! empty($data->lampiran_fasgub))
                <a class="mx-3" href="{{ route('download.dokumen', ['jenis_file' => 'lampiran_fasgub', 'nama_file' => $data->lampiran_fasgub]) }}">{{ $data->lampiran_fasgub }}</a>
                @endif
                <input
                  type="file"
                  class="form-control"
                  name="lampiran_fasgub"
                  id="lampiran_fasgub"
                  placeholder=""
                />
              </div>
                ...
                <div class="card">
                  ...
                  <div class="card-body">
                    ...
                    <div class="item-timeline pb-4">
                      <h5 class="card-title d-flex justify-content-between">{{ $history->nama }} <span class="small">{{ $history->log_time }}</span></h5>
                      <ul>
                        <li><span>Nota Dinas PPK Sudah Sign</span><br><a href="{{ route('download.dokumen', ['jenis_file' => 'nota_dinas_ppk', 'nama_file' => $history->nota_dinas_ppk]) }}">{{ $history->nota_dinas_ppk }}</a></li>
                        <li><span>Matrix RAB Semula Menjadi</span><br><a href="{{ route('download.dokumen', ['jenis_file' => 'matrik_rab', 'nama_file' => $history->matrik_rab]) }}">{{ $history->matrik_rab }}</a></li>
                        <li><span>Dokumen Pendukung Lainnya</span><br><a href="{{ route('download.dokumen', ['jenis_file' => 'dokumen_pendukung', 'nama_file' => $history->dokumen_pendukung]) }}">{{ $history->dokumen_pendukung }}</a></li>
                      </ul>
                    </div>
                    @endforeach
                  </div>
              ...
              <div class="mb-3 pt-4">
                <div class="card">
                  ...
                  <div class="card-body">
                    ...
                    <div class="item-timeline pb-4">
                      <h5 class="card-title d-flex justify-content-between">{{ $history->nama }} <span class="small">{{ $history->created_at }}</span></h6>
...
<script>
  $(function() {
    $("#status_fasgub").on('change', function(){
      if ($("#status_fasgub option:selected").val() == '{{ \App\TicketStatus::DISETUJUI_FASGUB }}') {
...
function submitRevisifasgub()
{
  ...
  var id           = $('#id').val();
  var no_nota_fasgub  = $('#no_nota_fasgub').val();
  var catatan_fasgub  = $('#catatan_fasgub').val();
  var lampiran_fasgub = $('#lampiran_fasgub').prop('files')[0] || '';

  var form_data = new FormData();

  form_data.append('id', id);
  form_data.append('status_fasgub', status_fasgub);
  form_data.append('no_nota_fasgub', no_nota_fasgub);
  form_data.append('catatan_fasgub', catatan_fasgub);
  form_data.append('lampiran_fasgub', lampiran_fasgub);
