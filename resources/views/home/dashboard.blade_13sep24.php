...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    ...
    <body>
        @php
            $persen_realisasi = $realisasi_data/$anggaran*100;
            $persen_selisih = $selisih/$anggaran*100;
            $persen_pagu_pusat      = $pagu_pusat/$anggaran*100;
            $persen_realisasi_pusat = $realisasi_anggaran/$pagu_pusat*100;
            ...
            $persen_realisasi_rm    = $realisasi_anggaran/$rm_pusat*100;
            $persen_sisa_rm         = $sisa_rm_pusat/$rm_pusat*100;

            $realisasi_rm_pusat     = $realisasi_anggaran-$realisasi_phln;
                    ...
                    <div class="col-md-4">
                        <div class="card card-dongker text-center">
                            <div class="card-body position-relative">
                                ...
                                <p class="h5">Rp {{number_format($pagu_anggaran,2,',','.')}}<br>&nbsp;</p>
                    ...
                    <div class="col-md-4">
                        <div class="card card-dongker text-center">
                            <div class="card-body">
                                ...
                                <p class="h5">Rp {{number_format($realisasi_data,2,',','.')}}<br>({{number_format($persen_realisasi,2,',','.')}}%)</p>
                                <div class="progress mb-2" role="progressbar" aria-label="Success example 4px high" aria-valuenow="25" aria-valuemin="0" aria-valuemax="100" style="height: 4px">
                    ...
                    <div class="col-md-6 px-5">
                        ...
                        <div class="row fw-semibold">
                            <div class="col-md-4 text-center">
                                @foreach ($realisasi as $realisasis )
                                    
                                
                                <h4 class="h6">PUSAT (Total)</h4>
                                @if($realisasis->bukti_ref)
                                    @php
                                        $excelPath = storage_path('app/public/bukti_ref/' . $realisasis->bukti_ref);
                                        $reader = \PhpOffice\PhpSpreadsheet\IOFactory::createReader('Xlsx');
                                        $spreadsheet = $reader->load($excelPath);
                                        $worksheet = $spreadsheet->getActiveSheet();
                                        $an5Value = $worksheet->getCell('AN5')->getValue();
                                    @endphp
                                <p>Rp {{ number_format($an5Value, 0, ',', '.') }}</p>
                                @else
                                    N/A
                                @endif
                            ...
                            <div class="col-md-4 text-center">
                                ...
                                <p>@if($realisasis->bukti_ref)
                                    @php
                                        $an7Value = $worksheet->getCell('AN7')->getValue();
                                    @endphp
                                    Rp {{ number_format($an7Value, 0, ',', '.') }}
                                @else
                                    N/A
                                @endif</p>
                                <div class="chart-box">
